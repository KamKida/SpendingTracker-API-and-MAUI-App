	...
	public class SpendingService : ISpendingService
	{
		...
		public async Task<HttpResponseMessage> Get10(SpendingFilterRequest request, bool useDatesFromToo = false)
		{
			...
			try
			{
				...
				_logger.LogInformation(
					"Wynik pobierania 10 wydatków: StatusCode={StatusCode}",
					response.StatusCode
				);
		...
		{
			_logger.LogInformation(
				"Rozpoczynam dodawanie wydatku. Amount={Amount}",
				request.Amount
			);
			...
			try
			{
				...
				_logger.LogInformation(
					"Wynik dodawania wydatku: StatusCode={StatusCode}",
					response.StatusCode
				);
			...
			catch (HttpRequestException httpEx)
			{
				_logger.LogError(
					httpEx,
					"Błąd HTTP podczas dodawania wydatku. Amount={Amount}",
					request.Amount
				);
				throw;
			}
			catch (Exception ex)
			{
				_logger.LogError(
					ex,
					"Nieoczekiwany błąd podczas dodawania wydatku. Amount={Amount}",
					request.Amount
				);
				throw;
			}
		...
		public async Task<HttpResponseMessage> DeleteSpending(Guid id)
		{
			...
			try
			{
				...
				_logger.LogInformation(
					"Wynik usuwania wydatku. SpendingId={SpendingId}, StatusCode={StatusCode}",
					id,
					response.StatusCode
				);
		...
		{
			_logger.LogInformation(
				"Rozpoczynam edycję wydatku. SpendingId={SpendingId}, Amount={Amount}",
				request.Id,
				request.Amount
			);
			...
			try
			{
				...
				_logger.LogInformation(
					"Wynik edycji wydatku. SpendingId={SpendingId}, StatusCode={StatusCode}",
					request.Id,
					response.StatusCode
				);
